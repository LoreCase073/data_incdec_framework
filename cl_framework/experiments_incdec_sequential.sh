...
do
    ...
    -op runs_trainings/from_checkpoint_sgd/efc/data_substitution/damp_001/lamb_1/seed_$i/ \
        --approach incdec_efc --pipeline baseline --n_accumulation 4 --seed $i --nw 4 \
        --freeze_bn no --freeze_backbone no --early_stopping_val 10 --weight_decay 5e-4 \
        --restore_initial_parameters no \
        --efc_lambda 1 --damping 0.01 \
        --stop_first_task no \
